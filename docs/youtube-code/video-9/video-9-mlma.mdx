...
PlaceKalshiOrder(action, marketTicker, side, price, size, clientOrderId := "") {
    ...
    if (res.status = 401 || res.status = 403) {
        Log("ERROR: API key or private key invalid (HTTP " res.status "). Please fix credentials and restart.")
        stopCode()
    }
    ...
    ; For buys: cancel the order if position not yet confirmed so it can't fill later and cause a duplicate
