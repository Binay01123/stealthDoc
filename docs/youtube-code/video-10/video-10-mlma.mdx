...
EnsureKalshiCryptoShardFunding(context) {
    ...
    if (!responseAmbiguous && res.status != 200) {
        if (baseFundingReady) {
            kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
            Log("WARN: " context " Additional agent reserve rejected | HTTP " res.status " | " KalshiApiMessage(res.body) " | continuing because shard 2 met the base requirement before this optional transfer")
            return true
        }
        kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
        Log("ERROR: " context " Shard 2 top-up rejected | HTTP " res.status " | " KalshiApiMessage(res.body))
        return false
    }
