...
![Screen map showing TradingView and Kalshi](./2026-02-16-23-43-52.png)
...
(() => {
  ...
  hud.querySelector("#nextSession").style.padding = "10px 16px";
  ...
  // ================= NEXT SESSION =================
  function findSessionPills() {
    const timeRe = /\b\d{1,2}:\d{2}\s*(AM|PM)\b/i;
    return [...document.querySelectorAll('button, a')].filter(el => {
      if (hud.contains(el)) return false;
      const t = (el.textContent || "").trim();
      if (!timeRe.test(t)) return false;
      if (/\bmore\b/i.test(t) || /\bpast\b/i.test(t)) return false;
      if (!el.classList.contains("inline-flex")) return false;
      return true;
    });
  }

  function findActiveSessionIndex(pills) {
    for (let i = 0; i < pills.length; i++) {
      if (pills[i].classList.contains("bg-fill-x10")) return i;
      if (pills[i].classList.contains("text-text-inverse")) return i;
    }
    return -1;
  }

  function clickNextSession() {
    const pills = findSessionPills();
    if (!pills.length) return log("NEXT SESSION: no session pills found");

    const activeIdx = findActiveSessionIndex(pills);
    if (activeIdx === -1) return log("NEXT SESSION: active not found");

    const nextIdx = activeIdx + 1;
    if (nextIdx >= pills.length) return log("NEXT SESSION: already on last visible session");

    const nextPill = pills[nextIdx];
    log("NEXT SESSION: clicking " + (nextPill.textContent || "").trim());
    pointerClick(nextPill);
  }

  hud.querySelector("#nextSession").onclick = () => clickNextSession();

  // ================= AUTO-DISMISS "0 of N" DONE PANEL =================
  function autoDismissLostPanel() {
    const spans = document.querySelectorAll("span");
    for (const s of spans) {
      if (hud.contains(s)) continue;
      const t = (s.textContent || "").trim();
      if (!/^0\s+of\s+\d+$/.test(t)) continue;

      const panel = s.closest("div.sticky") || s.closest("div");
      if (!panel) continue;

      const btns = panel.querySelectorAll("button");
      for (const b of btns) {
        if (norm(b.textContent) === "done") {
          log("AUTO-DISMISS: clicking Done (0 of " + t.split("of")[1].trim() + ")");
          pointerClick(b);
          return;
        }
      }
  ...
  STATE.timerInterval = setInterval(() => {
    try { updateTimer(); updatePosition(); autoDismissLostPanel(); } catch {}
  }, 1000);
...
loop {
  ...
  loop{
    read_areas()
  } until (toNumber(area[1]) > triggerPoint || toNumber(area[2]) > triggerPoint )
