...
;DAILY TARGETS
;=================
Daily_Profit_Target := 400     ; stop when aggregate Kalshi balance change is greater than this; 0 disables
Daily_Loss_Target := -200       ; stop when aggregate Kalshi balance change is less than this; 0 disables
...
orderSizeValid := IsNumericStr(orderSize) && (orderSize + 0) > 0
if (!orderSizeValid) {
    Log("ERROR: orderSize is '" orderSize "'. It needs to be a positive number of contracts, for example 1 or 2.5.")
    settingsValid := false
}
...
kalshiDailyStartBalance := ""
kalshiDailyLastBalance := ""
kalshiDailyBalanceLastAttemptTick := 0
...
loop {
    for _, asset in assets {
        ...
        if (!assetSessionQuarter.HasKey(asset) || assetSessionQuarter[asset] != sessionKey) {
            ...
            assetLastPriceLogTick[asset] := 0
            LogOnceReset(asset)
            Log(asset " New session (" snapshot.marketTicker ", shard " snapshot.exchangeIndex ")")
            kalshiHasUntrackedPosition := false

            if (!EnsureKalshiShardFunding(asset, snapshot.exchangeIndex)) {
                assetPhase[asset] := "FUNDING_FAILED"
                Log(asset " Shard funding unavailable; skipping this session")
                continue
            }

            if (snapshot.marketTicker != "" && HasExistingPosition(snapshot.marketTicker, snapshot.exchangeIndex)) {
        ...
        existingPositionSize := GetKalshiPositionSize(snapshot.marketTicker, 3, snapshot.exchangeIndex)
        ...
        buyConfirmed := false
        loop, 4 {
            if (A_Index > 1 && HasExistingPosition(snapshot.marketTicker, snapshot.exchangeIndex)) {
            ...
            if BuyPosition(asset, snapshot.marketTicker, side, entryPrice, snapshot.exchangeIndex) {
                buyConfirmed := true
                break
            }
        ...
        if (buyConfirmed) {
            ...
            if (posSize = "" || posSize <= 0)
                posSize := GetKalshiPositionSize(snapshot.marketTicker, 3, snapshot.exchangeIndex)
            if (posSize = "" || posSize <= 0)
                posSize := orderSize + 0
            ...
            positions[asset] := { side: side, entry: fillPrice + 0, entryFees: entryFees, size: posSize, expectedSize: posSize, accountingReady: accountingReady, entryOrderId: orderId, takeProfitTarget: takeProfitTarget, ticker: snapshot.marketTicker, exchangeIndex: snapshot.exchangeIndex, quarterIndex: CurrentQuarterIndex(), lastMark: initialMark }
            assetPhase[asset] := "IN_POSITION"
            assetLastPriceLogTick[asset] := 0
            if (accountingReady)
...
BuyPosition(asset, marketTicker, side, price, exchangeIndex := -1) {
    global orderSize
    return PlaceKalshiOrder("BUY", marketTicker, side, price, orderSize, "", exchangeIndex)
}
...
SellPosition(asset, side, price, reason, clientOrderId := "") {
    ...
    exchangeIndex := positions[asset].HasKey("exchangeIndex") ? positions[asset].exchangeIndex : -1
    return PlaceKalshiOrder("SELL", positions[asset].ticker, side, price, size, clientOrderId, exchangeIndex)
}

HasExistingPosition(marketTicker, exchangeIndex := "") {
    size := GetKalshiPositionSize(marketTicker, 1, exchangeIndex)
    return (size != "" && size > 0)
}

GetKalshiPositionSize(marketTicker, maxAttempts := 1, exchangeIndex := "") {
    if (marketTicker = "")
        return ""

    endpoint := "/trade-api/v2/portfolio/positions?ticker=" marketTicker
    if (exchangeIndex != "")
        endpoint .= "&exchange_index=" NormalizeKalshiExchangeIndex(exchangeIndex)
...
RunPendingKalshiDailyTargetCheck() {
    global kalshiDailyTargetCheckPending, Daily_Profit_Target, Daily_Loss_Target, kalshiDailyStartBalance, kalshiDailyLastBalance, kalshiDailyBalanceLastAttemptTick
    if (!kalshiDailyTargetCheckPending || KalshiHasOpenExposure())
        return false

    nowTick := A_TickCount
    if (kalshiDailyBalanceLastAttemptTick > 0 && (nowTick - kalshiDailyBalanceLastAttemptTick) < 2000)
        return false
    kalshiDailyBalanceLastAttemptTick := nowTick

    currentBalance := GetKalshiAggregateBalanceDollars(3)
    if (currentBalance = "" || kalshiDailyStartBalance = "") {
        LogOnce("ACCOUNT", "DAILY_BALANCE_UNAVAILABLE", "Daily target check waiting for aggregate Kalshi balance")
        return false
    }

    kalshiDailyLastBalance := currentBalance + 0
    pnl := kalshiDailyLastBalance - (kalshiDailyStartBalance + 0)

    kalshiDailyTargetCheckPending := false
    Log("Daily balance check | start $" FmtMoney(kalshiDailyStartBalance) " | current $" FmtMoney(kalshiDailyLastBalance) " | change " FmtPnl(pnl))
    UpdateKalshiPnlLogStreams()
    if ((Daily_Profit_Target + 0) > 0 && pnl > (Daily_Profit_Target + 0)) {
        Log("Daily profit target reached | aggregate balance change " FmtPnl(pnl) " > " FmtPnl(Daily_Profit_Target) " | stopping bot")
        stopCode()
        return true
    }
    if ((Daily_Loss_Target + 0) < 0 && pnl < (Daily_Loss_Target + 0)) {
        Log("Daily loss target reached | aggregate balance change " FmtPnl(pnl) " < " FmtPnl(Daily_Loss_Target) " | stopping bot")
        stopCode()
        return true
    }
    return false
...
PlaceKalshiOrder(action, marketTicker, side, price, size, clientOrderId := "", exchangeIndex := -1) {
    global enableLiveOrders, kalshiApiKeyId, kalshiPrivateKeyPath, restDelay, entryDiff, exitDiff, kalshiLastOrderResult
    exchangeIndex := NormalizeKalshiExchangeIndex(exchangeIndex)
    kalshiLastOrderResult := { action: action, ticker: marketTicker, exchangeIndex: exchangeIndex, side: side, apiAction: "", apiSide: "", cents: "", price: "", contractCents: "", fillPrice: "", fillSummary: NewKalshiFillSummary(), positionSize: "", timeInForce: "", size: size, status: "", body: "", orderId: "", clientOrderId: clientOrderId, reason: "not submitted" }
    ...
    if (isBuy && orderId != "") {
        remainingBefore := GetKalshiOrderRemainingCount(orderId, remainingQueryOk, exchangeIndex)
        cancelNeeded := !remainingQueryOk || remainingBefore > 0.000001
        if (cancelNeeded) {
            cancelConfirmed := CancelKalshiOrder(orderId, exchangeIndex)
            Sleep 500
            remainingAfter := GetKalshiOrderRemainingCount(orderId, remainingAfterQueryOk, exchangeIndex)
    ...
    positionSize := GetKalshiPositionSize(marketTicker, 3, exchangeIndex)
    if (positionSize = "") {
        kalshiLastOrderResult.fatal := true
...
KalshiLastOrderApiSummary() {
    ...
    exchangeIndex := KalshiResultValue(kalshiLastOrderResult, "exchangeIndex", "")
...
NormalizeKalshiExchangeIndex(exchangeIndex) {
    exchangeIndex := Trim(exchangeIndex)
    if !RegExMatch(exchangeIndex, "^-?\d+$")
        return -1
    exchangeIndex += 0
    return (exchangeIndex >= -1) ? exchangeIndex : -1
}

KalshiBalanceDollarsFromBody(body) {
    balanceDollars := JsonField(body, "balance_dollars")
    if !IsNumericStr(balanceDollars)
        return ""
    return balanceDollars + 0
}

GetKalshiBalanceSnapshot(exchangeIndex := "", maxAttempts := 1) {
    snapshot := { ok: false, balance: "", status: "", body: "", userMissing: false }
    endpoint := "/trade-api/v2/portfolio/balance"
    if (exchangeIndex != "") {
        exchangeIndex := NormalizeKalshiExchangeIndex(exchangeIndex)
        if (exchangeIndex < 0)
            return snapshot
        endpoint .= "?exchange_index=" exchangeIndex
    }
    ...
    while (attempt <= maxAttempts) {
        ...
        if IsObject(res) {
            ...
            if (res.status = 200) {
                balanceDollars := KalshiBalanceDollarsFromBody(res.body)
                if (balanceDollars != "") {
                    snapshot.ok := true
            ...
            if (exchangeIndex != "" && InStr(errorCode, "user_not_found") = 1) {
                ; A newly introduced shard has no user record until its first transfer.
                snapshot.ok := true
                snapshot.balance := 0
                snapshot.userMissing := true
                return snapshot
            }
        }
        if (attempt < maxAttempts)
...
GetKalshiAggregateBalanceDollars(maxAttempts := 1) {
    snapshot := GetKalshiBalanceSnapshot("", maxAttempts)
    return (IsObject(snapshot) && snapshot.ok) ? snapshot.balance + 0 : ""
}

EnsureKalshiShardFunding(asset, destinationExchangeIndex) {
    global enableLiveOrders, orderSize, assets, kalshiShardFundingSessionStatus
    if (!enableLiveOrders)
        return true

    destinationExchangeIndex := NormalizeKalshiExchangeIndex(destinationExchangeIndex)
    if (destinationExchangeIndex = 0)
        return true
    if (destinationExchangeIndex < 0) {
        Log("ERROR: " asset " Market did not provide a valid exchange shard; automatic funding skipped")
        return false
    }

    fundingSessionKey := destinationExchangeIndex "|" CurrentQuarterIndex()
    ...
    ; Reserve the $1 maximum payout per contract for every enabled asset that
    ; can enter during the same interval. With one asset this is orderSize * $1.
    enabledAssetCount := IsObject(assets) ? assets.Length() : 1
    if (enabledAssetCount < 1)
        enabledAssetCount := 1
    requiredDollars := (orderSize + 0) * enabledAssetCount
    destination := GetKalshiBalanceSnapshot(destinationExchangeIndex, 3)
    if (!IsObject(destination) || !destination.ok) {
        kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
        Log("ERROR: " asset " Could not read shard " destinationExchangeIndex " balance | HTTP " (IsObject(destination) ? destination.status : "no response"))
        return false
    }
    ...
    if (baseFundingReady) {
        ; A sufficient balance may belong to another running agent. Add this
        ; process's own orderSize * $1 reserve instead of reusing it.
        ...
        ; Preserve the existing behavior when shard 2 is underfunded: transfer
        ; only the shortfall needed to reach this script's requirement.
    ...
    source := GetKalshiBalanceSnapshot(0, 3)
    if (!IsObject(source) || !source.ok) {
        if (baseFundingReady) {
            kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
            Log("WARN: " asset " Could not read source shard 0 for the additional agent reserve; continuing because shard " destinationExchangeIndex " met the base requirement before this optional transfer")
            return true
        }
        kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
        Log("ERROR: " asset " Could not read source shard 0 balance; automatic funding skipped")
        return false
    }
    if ((source.balance + 0) < (transferDollars - 0.000001)) {
        if (baseFundingReady) {
            kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
            Log("WARN: " asset " Source shard 0 lacks the $" FmtMoney(transferDollars) " additional agent reserve; continuing because shard " destinationExchangeIndex " met the base requirement before this optional transfer")
            return true
        }
        kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
        Log("ERROR: " asset " Shard top-up needs $" FmtMoney(transferDollars) " from shard 0 but only $" FmtMoney(source.balance) " is available")
        return false
    }
    ...
    payload := "{"
        ...
        . """destination_exchange_shard"":" destinationExchangeIndex ","
        . """source_subaccount"":0,"
    ...
    if (!responseAmbiguous && res.status != 200) {
        if (baseFundingReady) {
            kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
            Log("WARN: " asset " Additional agent reserve rejected | HTTP " res.status " " KalshiApiMessage(res.body) " | continuing because shard " destinationExchangeIndex " met the base requirement before this optional transfer")
            return true
        }
        kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
        Log("ERROR: " asset " Shard " destinationExchangeIndex " top-up rejected | HTTP " res.status " " KalshiApiMessage(res.body))
        return false
    }

    if (responseAmbiguous) {
        Log("WARN: " asset " Shard top-up response was ambiguous; checking destination balance before allowing orders")
    } else {
        transferId := JsonField(res.body, "transfer_id")
        Log(asset " Shard " destinationExchangeIndex " " transferReason " requested | $" FmtMoney(transferDollars) " | transfer_id=" KalshiShortLogText(transferId, 28))
    }

    ; Transfers are asynchronous. A required shortfall must be confirmed. An
    ; optional extra reserve is best-effort because the base balance was ready.
    Loop, 20 {
        Sleep 500
        destination := GetKalshiBalanceSnapshot(destinationExchangeIndex, 1)
        if (IsObject(destination) && destination.ok && (destination.balance + 0) >= (confirmationTargetDollars - 0.000001)) {
            kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
            Log(asset " Shard " destinationExchangeIndex " funding confirmed | $" FmtMoney(destination.balance) " available")
            return true
        }
    }
    ...
    if (baseFundingReady) {
        kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
        Log("WARN: " asset " Additional agent reserve was not confirmed within 10 seconds; continuing because shard " destinationExchangeIndex " met the base requirement before this optional transfer. Order placement will report if balance is no longer sufficient.")
        return true
    }

    kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
    Log("ERROR: " asset " Shard " destinationExchangeIndex " top-up was not confirmed within 10 seconds; skipping this session")
    return false
}

CancelKalshiOrder(orderId, exchangeIndex := -1) {
    if (orderId = "")
        return false
    exchangeIndex := NormalizeKalshiExchangeIndex(exchangeIndex)
    res := KalshiSignedRequest("DELETE", "/trade-api/v2/portfolio/events/orders/" orderId "?exchange_index=" exchangeIndex)
    return IsObject(res) && (res.status = 200 || res.status = 204)
}

GetKalshiOrderRemainingCount(orderId, ByRef queryOk, exchangeIndex := -1) {
    queryOk := false
    if (orderId = "")
        return ""
    ...
    exchangeIndex := NormalizeKalshiExchangeIndex(exchangeIndex)
    res := KalshiSignedRequest("GET", "/trade-api/v2/portfolio/orders/" orderId "?exchange_index=" exchangeIndex)
    if !IsObject(res) || (res.status != 200)
        return ""
...
StartupCredentialCheck() {
    global enableLiveOrders, kalshiApiKeyId, kalshiPrivateKeyPath, kalshiSignerPath, kalshiDailyStartBalance, kalshiDailyLastBalance
    ...
    if (apiReady && keyReady && signerReady) {
        ...
        if IsObject(res) && (res.status = 200) {
            aggregateBalance := KalshiBalanceDollarsFromBody(res.body)
            if (aggregateBalance = "") {
                Log("ERROR: Credentials worked, but aggregate Kalshi balance could not be read. Daily safety limits cannot be initialized.")
                stopCode()
                return
            }
            kalshiDailyStartBalance := aggregateBalance + 0
            kalshiDailyLastBalance := aggregateBalance + 0
            Log("Credentials processed successfully | aggregate balance $" FmtMoney(aggregateBalance) " | daily baseline set")
            return
        }
...
GetKalshiMarketSnapshot(asset, maxRetries := 4) {
    ...
    while (attempt <= maxRetries) {
        ...
        if (body != "") {
            marketTicker := GetFirstNonEmptyJsonField(body, ["ticker"])
            exchangeIndex := GetFirstNonEmptyJsonField(body, ["exchange_index"])
            yesPrice := GetKalshiDollarPrice(body, "yes_ask_dollars", "yes_ask")
            noPrice  := GetKalshiDollarPrice(body, "no_ask_dollars", "no_ask")
            ...
            if (marketTicker != "") {
                ...
                obj.closeTime := closeIso
                obj.marketTicker := marketTicker
                obj.exchangeIndex := NormalizeKalshiExchangeIndex(exchangeIndex)
                assetSnapshotCache[asset] := obj
                assetMarketRefreshTick[asset] := nowTick
                if (RefreshKalshiBestAsks(obj) || (obj.up != "" && obj.down != ""))
                    return obj
