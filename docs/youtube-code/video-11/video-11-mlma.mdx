...
loop {
        ...
        if IsObject(pendingPair) {
            if !PairSessionEnded(pendingPair, btcSnap, ethSnap) {
                ...
                if PairAnyPositionOpen(pendingPair) {
                    ...
                    assetLastPriceLogTick["PAIR"] := 0
                    LogOnceReset("PAIR")
                    Log("PAIR New session | BTC " btcSnap.marketTicker " | ETH " ethSnap.marketTicker " | shard " kalshiCryptoExchangeIndex)

                    if (!EnsureKalshiCryptoShardFunding("PAIR")) {
                        pairPhase := "FUNDING_FAILED"
                        Log("PAIR Shard funding unavailable; skipping this session")
                    } else if (HasExistingPosition(btcSnap.marketTicker) || HasExistingPosition(ethSnap.marketTicker)) {
                        pairPhase := "HAS_POSITION"
                        Log("PAIR Existing BTC or ETH position found, waiting for next session")
                    }
                }
    ...
    if (pairPhase = "FUNDING_FAILED") {
        LogOnce("PAIR", "FUNDING_FAILED", "PAIR Shard funding failed, waiting for next session")
        Random, pollJitterMs, 0, 250
        sleepMs := pollIntervalMs + pollJitterMs
        Sleep %sleepMs%
        continue
...
SubmitKalshiIndividualOrders(action, legs, overridePrice := "", skipExisting := true) {
    ...
    for _, leg in legs {
        ...
        responseText := "ORDER " apiAction " " leg.asset " " leg.side " " leg.ticker " shard=2"
...
EnsureKalshiCryptoShardFunding(context) {
    global enableLiveOrders, orderSize, assets, shots, kalshiCryptoExchangeIndex, kalshiShardFundingSessionStatus
    if (!enableLiveOrders)
        return true
    ...
    legCount := IsObject(assets) ? assets.Length() : 2
    if (legCount < 1)
        legCount := 1
    shotCount := (shots + 0 > 0) ? shots + 0 : 1
    requiredDollars := (orderSize + 0) * legCount * shotCount
    ...
    if (baseFundingReady) {
        ; Arb's extra per-agent reserve is one orderSize for every configured shot.
        transferCenticents := Ceil(((orderSize + 0) * shotCount * 10000) - 0.000001)
    ...
    if (!responseAmbiguous && res.status != 200) {
        if (baseFundingReady) {
            kalshiShardFundingSessionStatus[fundingSessionKey] := "READY"
            Log("WARN: " context " Additional agent reserve rejected | HTTP " res.status " | " TruncateLogText(res.body) " | continuing because shard 2 met the base requirement before this optional transfer")
            return true
        }
        kalshiShardFundingSessionStatus[fundingSessionKey] := "FAILED"
        Log("ERROR: " context " Shard 2 top-up rejected | HTTP " res.status " | " TruncateLogText(res.body))
        return false
    }
...
CancelKalshiOrder(orderId) {
    ...
    if (orderId = "")
        return true
    res := KalshiSignedRequest("DELETE", "/trade-api/v2/portfolio/events/orders/" orderId "?exchange_index=" kalshiCryptoExchangeIndex)
    if !IsObject(res) {
